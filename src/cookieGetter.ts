...
export default class CookieGetter {
  async getCookie(url: string, endUrl?: string): Promise<string> {
    ...
    const elem = await Promise.race([page.waitForSelector('input[type="email"]'), page.waitForSelector('input[type="username"]'), page.waitForSelector('input[type="text"]')]);
    ...
    if (!!isHidden && isHidden !== null)
    {
      ...
      const buttonLocator = await Promise.race([page.waitForSelector('text="Next"'), page.waitForSelector('text="Submit"'), page.waitForSelector('text="Yes"'), page.waitForSelector('text="Login"'), page.waitForSelector('text="Yes"')])
        //@ts-ignore
